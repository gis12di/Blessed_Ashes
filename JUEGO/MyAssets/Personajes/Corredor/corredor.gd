...
func _process(delta):
	...
	if Global.CenithAlive:
		is_corredor_chase = true
	elif !Global.CenithAlive:
		is_corredor_chase = false
		
	Global.corredorDamageAmount = damage_to_deal
	Global.corredorDamageZone = $corredorDealDamageArea
...
func move(delta):
	if !dead:
		if !is_corredor_chase:
			velocity += dir * speed * delta
		elif is_corredor_chase and !taking_damage:
...
func _on_corredor_deal_damage_area_area_entered(area: Area2D) -> void:
...
func _on_corredor_hitbox_area_entered(area: Area2D) -> void:
